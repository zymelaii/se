...
#ifdef DEBUG
#include <stdio.h>
#define SE_INFO(e, args...) printf("info: " e "\n", ##args)
#define SE_WARN(e, args...) printf("warn: " e "\n", ##args)
#define SE_ERROR(e, args...) printf("error: " e "\n", ##args)
#else
#define SE_INFO(...)
#define SE_WARN(...)
#define SE_ERROR(...)
...
unit_t *toks2rpn(token_t *tokens, int ntok, int *psize, int *err)
{
	...
	int size = ntok, i = 0, error = ECE_DONE;
	...
	for (i = 0; i < ntok; ++i)
	{
		...
		if (error != ECE_DONE) break;
	}

	if (error == ECE_DONE) while (q != size)
	{
...
seus_t rpn2seus(unit_t *units, int n, int *err)
{
	...
	for (i = 0, j = 0; i < n; ++i)
	{
		...
		switch (e->type & 0xff)
		{
			...
			case OP_ARR:
			{
				...
				if (ef > ss[p].sframe)
				{
					SE_ERROR("SyntaxError: missing ','");
					goto _error;
				} else if (ef < ss[p].sframe)
				{
					if (ss[p].accept > 0)
					{
						SE_ERROR("SyntaxError: too many commas");
						goto _error;
					} else
					{
						nele = 0;
				...
				if (nele == 0)
				{
					if (op == OP_IDX)
					{
						SE_ERROR("IndexError: missing index");
						goto _error;
					} else
					{
						++ef;
				...
				} else
				{
					...
					if (vf < -1)
					{
						SE_ERROR("SyntaxError: invalid syntax (2)");
						goto _error;
					}
				}
				...
				if (op == OP_IDX)
				{
					...
					if (ef < 0)
					{
						SE_ERROR("IndexError: missing array");
						goto _error;
					}
				}
				...
				if (op == OP_ARG)
				{
					...
					if (ef < 0)
					{
						SE_ERROR("RuntimeError: missing function");
						goto _error;
					}
				}
				...
				// if (p < 0)
				// {
				// 	SE_ERROR("SyntaxError: invalid syntax (3)");
				// 	goto _error;
				// }
			}
			continue;
			...
			case OP_LNOT:
			{
				if (ef < ss[p].sframe)
				{
					SE_ERROR("SyntaxError: missing operand");
					goto _error;
				}
			}
			continue;
	...
	if (ef != 0)
	{
		SE_ERROR("SyntaxError: invalid syntax (4)");
		goto _error;
	}
